...
foreach my $proj (@projects){
	...
	if (( -e $mutation_file) && (-e $gene_level_calls) && (-e $purity_seg)  && (-e $signature_calls) && (-e $wgd_file)){
		...
		#`mkdir $new_proj`;
		print "All the input files exist in project Proj_$proj for Rscript portal.R\n";
		...
		`cp $mutation_file $new_proj`;
		 ...
		 #Change the IDS
		 my $new_gene_file   		       = $new_proj."/Proj_".$proj."_ccs_filtered_genelevel_cna_043019.txt";
		...
		$hash_log{"MSI"} = {%hash3};
		 ...
		 `cp $purity_seg  $new_proj`;
		  ...
		  #Change the IDS
		  my $time                                   = getLogTime();
		  my $new_purityseg_file                     = $new_proj."/Proj_".$proj."_purity_seg_061119.txt";
...
sub run_portal_script{
	...
	my $cmd = "Rscript  port1.R $$proj $$gene_file $$mut_file  $$wgd_file  $$purity_file $$sig_file $$msi_file $$sample_f $$patient_f  $$out" ;
	print "$cmd\n";
        `$cmd`;	
		...
		foreach my $h(@tumor_ids){
			 ...
			 if ($a[15] =~ m/s_/){
			  ...
			  if ($a[0] eq $tumor){
		       ...
		       }elsif($$mut_file =~ m/fil_top3list_022119.txt/){
			       $a[0] =~ s/^\s*(.*?)\s*$/$1/;
			       if  ($a[0] eq $tumor){
				s/$a[0]/$wes/g;
				print WRITE $_."\n";
